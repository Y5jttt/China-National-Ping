...
func configApiRoutes() {
	...
	//全国延迟详细信息API
	http.HandleFunc("/api/mapping_detail.json", func(w http.ResponseWriter, r *http.Request) {
		if !AuthUserIp(r.RemoteAddr) && !AuthAgentIp(r.RemoteAddr, true) {
			o := "Your ip address (" + r.RemoteAddr + ")  is not allowed to access this site!"
			http.Error(w, o, 401)
			return
		...
		w.Header().Set("Content-Type", "application/json")
		RenderJson(w, funcs.MapDetailStatus)
	})

	//省份IP历史延迟API
	http.HandleFunc("/api/province_history.json", func(w http.ResponseWriter, r *http.Request) {
		if !AuthUserIp(r.RemoteAddr) && !AuthAgentIp(r.RemoteAddr, true) {
			o := "Your ip address (" + r.RemoteAddr + ")  is not allowed to access this site!"
			http.Error(w, o, 401)
			return
		...
		if province == "" {
			http.Error(w, "Missing province", 400)
			return
		}

		// Get time range in hours (default 1 hour)
		hours := 1
		if h := r.FormValue("hours"); h != "" {
		...
		startTime := time.Now().Add(-time.Duration(hours) * time.Hour).Format("2006-01-02 15:04")
		endTime := time.Now().Format("2006-01-02 15:04")

		// Get IPs for this province from mapping_detail
		detail := funcs.MapDetailStatus[province]
		...
		type ProvinceHistory struct {
			...
			IPs      []IPHistory `json:"ips"`
		}

		result := ProvinceHistory{Province: province, StartTime: startTime, EndTime: endTime}

		// Collect all IPs for this province
		type ipKey struct {
			ip      string
			telecom string
			city    string
		}
		ipMap := make(map[ipKey][]g.IPDetail)

		if detail.Ctcc != nil {
			for city, ips := range detail.Ctcc {
				for _, ip := range ips {
					k := ipKey{ip: ip.IP, telecom: "ctcc", city: city}
					ipMap[k] = ips
				}
			}
		}
		if detail.Cucc != nil {
			for city, ips := range detail.Cucc {
				for _, ip := range ips {
					k := ipKey{ip: ip.IP, telecom: "cucc", city: city}
					ipMap[k] = ips
				}
			}
		}
		if detail.Cmcc != nil {
			for city, ips := range detail.Cmcc {
				for _, ip := range ips {
					k := ipKey{ip: ip.IP, telecom: "cmcc", city: city}
					ipMap[k] = ips
				}
			}
		}

		// Get history for each IP from database
		for k, ips := range ipMap {
			for _, ipInfo := range ips {
				...
				querySql := "SELECT avgdelay, losspk, logtime FROM pinglog WHERE target = '" + ipInfo.IP + "' AND logtime >= '" + startTime + "' AND logtime <= '" + endTime + "' ORDER BY logtime ASC"
				rows, err := g.Db.Query(querySql)
				if err == nil {
					for rows.Next() {
						...
						var losspk int
						var logtime string
						rows.Scan(&avgdelay, &losspk, &logtime)
						delayHistory = append(delayHistory, avgdelay)
						lossHistory = append(lossHistory, losspk)
						timeHistory = append(timeHistory, logtime)
						if avgdelay < 2000 {
							totalDelay += avgdelay
							count++
						}
				...
				result.IPs = append(result.IPs, IPHistory{
					IP:       ipInfo.IP,
					Telecom:  k.telecom,
					City:     k.city,
	...
	http.HandleFunc("/api/saveconfig.json", func(w http.ResponseWriter, r *http.Request) {
		...
		for _, provVal := range nconfig.Chinamap {
			for _, cityMap := range provVal {
				for _, ips := range cityMap {
					for _, ip := range ips {
						if ip != "" && !ValidIP4(ip) {
		...
		if nconfig.Alert["SendEmailPassword"] == "" {
